...
def _btn_gold(parent, text, command, **kwargs):
    ...
    return ctk.CTkButton(
        ...
        text_color=FG, border_width=1, border_color=ACCENT_LT,
        font=FONT_LABEL, corner_radius=16, **kwargs
    )
...
def _btn_outline(parent, text, command, **kwargs):
    ...
    return ctk.CTkButton(
        parent, text=text, command=command,
        fg_color=GLASS, hover_color=GLASS_EMERALD,
        text_color=FG, border_color=BORDER_SOFT, border_width=1,
        font=FONT_BODY, corner_radius=16, **kwargs
    )
...
class RelayGuiApp:
    def __init__(self, root):
        ...
        self.api_key        = None  # set after desktop OAuth handoff

        self.log_visible    = False
        self.adv_visible    = False
        self.status_dots    = {}   # name → (dot_label, text_label)
    ...
    def _build_ui(self):
        ...
        top_glow = ctk.CTkFrame(outer, fg_color=PRIMARY_DK, height=8, corner_radius=0)
        top_glow.pack(fill="x")

        self._build_header(outer)
        self._build_status_bar(outer)

        # Two-column content with scroll
    ...
    def _build_status_bar(self, parent):
        bar = _card(parent, corner_radius=0, border_width=0, fg_color=BG_ELEVATED)
        ...
        for name in ["Bridge", "MT5", "Broker"]:
           f = ctk.CTkFrame(inner, fg_color=BG_CARD, corner_radius=18, border_width=1, border_color=BORDER)
    ...
    def _build_left(self, parent):
        # Plain frame — no scrolling; all content fits in the window
        ...
        self.pass_entry = _entry(login_card, self.password_var,
                                "Password", show="*")
        ...
        _btn_gold(login_card, "Sign In", self._sign_in,
               height=44).pack(fill="x", padx=18, pady=(0, 8))

        # OAuth row — Google + Facebook
        ...
        self.mt5_acct_entry = _entry(mt5_card, self.mt5_acct_var,
                                    "MT5 Account Number")
        self.mt5_acct_entry.pack(fill="x", padx=18, pady=(0, 8))

        self.mt5_pw_entry = _entry(mt5_card, self.mt5_pw_var,
                                  "MT5 Password", show="*")
        ...
        _chip(actions_card, "EXECUTION", GLASS_EMERALD, text_color=PRIMARY_LT).pack(anchor="w", padx=18, pady=(18, 8))
        _label(actions_card, "Actions", font=FONT_LABEL, color=FG).pack(anchor="w", padx=18)
        _label(actions_card, "Use local MT5 for direct execution or activate cloud mode for continuous trading.",
            color=FG_MUTED, font=FONT_SMALL).pack(anchor="w", padx=18, pady=(4, 12))

        self.vps_btn = _btn_gold(actions_card, "Enable VPS 24/7 Mode",
                                self.enable_managed_mode, height=38)
        self.vps_btn.pack(fill="x", padx=18, pady=(0, 8))
        ...
        _label(adv_toggle, "Advanced Routing", color=FG_SOFT,
        ...
        _label(log_toggle, "Execution Logs", color=FG_SOFT,
              font=FONT_SMALL).pack(side="left")
        ...
        self.log_box = ctk.CTkTextbox(
           ...
           font=FONT_MONO, corner_radius=16
        )
    ...
    def _build_right(self, parent):
        ...
        overview_card = _card(right, fg_color=BG_ELEVATED)
        ...
        self.summary_text = ctk.CTkTextbox(
           ...
           border_color=BORDER_SOFT, border_width=1,
           font=FONT_MONO, corner_radius=18,
        )
    ...
    def _auto_connect_if_cached(self):
        if self.user_id_var.get().strip() and self.password_var.get():
           if IS_WINDOWS:
               ...
               # Mac: refresh dashboard mirror automatically
    ...
    # ── Actions ───────────────────────────────────────────────────────────────
    def _sign_in(self):
        """Sign in button — starts relay on Windows, authenticates on Mac."""
    ...
    def _open_oauth(self, provider: str):
        """Start desktop OAuth handoff and poll for the returned API key."""
        ...
        def run_setup():
           ...
           if ok:
               ...
               self.root.after(0, lambda: self.vps_btn.configure(
                   ...
                   border_color=PRIMARY_LT,
                   text_color=FG, state="normal"
               ))
               threading.Thread(target=self._refresh_dashboard_summary,
                                daemon=True).start()
           else:
               self.update_status("VPS setup failed — check credentials")
               self.root.after(0, lambda: self.vps_btn.configure(
                   text="Enable VPS 24/7 Mode",
                   fg_color=GLASS_GOLD, hover_color=ACCENT_DK,
                   border_color=ACCENT_LT,
                   text_color=FG, state="normal"
               ))
    ...
    def stop_relay(self):
        ...
        if IS_WINDOWS and hasattr(self, "connect_btn"):
           self.connect_btn.configure(state="normal")
        self.vps_btn.configure(text="Enable VPS 24/7 Mode",
                              fg_color=GLASS_GOLD, hover_color=ACCENT_DK,
                              border_color=ACCENT_LT,
                              text_color=FG)
    ...
    def _create_tray_icon(self):
        if not pystray or not Image or not ImageDraw:
           return None
        img  = Image.new("RGB", (64, 64), color=(6, 14, 11))
        draw = ImageDraw.Draw(img)
        # Gold circle
        draw.ellipse((8, 8, 56, 56), fill=(232, 165, 32))
        menu = pystray.Menu(
